...
$homeDestino = "/corporativo/servicos/bbhive/relatorios/paragrafos/paragrafos.php";

if($_GET['acao']=="subir"){
	//recupera dados da troca
	$bbh_mod_par_codigo = $_GET['bbh_mod_par_codigo'];
	$bbh_mod_flu_codigo = $_GET['bbh_mod_flu_codigo'];
	$bbh_tip_flu_codigo = $_GET['bbh_tip_flu_codigo'];
    ...
    list($Result1, $rows, $totalRows) = executeQuery($bbhive, $database_bbhive, $updateSQLII);
	 
	//echo $bbh_mod_par_codigo."<hr>";
	//echo $codAnterior;
	echo "<var style='display:none'>OpenAjaxPostCmd('".$homeDestino."?bbh_tip_flu_codigo=".$bbh_tip_flu_codigo."&bbh_mod_flu_codigo=".$bbh_mod_flu_codigo."&Ts=".time()."','exibePar','&1=1','Atualizando dados...','loadOrdena','2','1');</var>";
	  exit;
}
