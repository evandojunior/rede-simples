...
//include($_SESSION['caminhoFisico']."/e-solution/servicos/bbhive/includes/autentica.php");
	...
	$homeDestinoII	= '/e-solution/servicos/bbhive/departamentos/campos_tabela/novo.php';
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
  ...
  //recuperando os tipos:
	...
	//--Ultimo ID
		$query_last_id = "SELECT max(bbh_cam_ind_codigo) bbh_cam_ind_codigo, max(bbh_cam_ind_ordem) bbh_cam_ind_ordem FROM bbh_campo_indicio limit 1";
        list($last_id, $row_last_id, $totalRows_Colunas) = executeQuery($bbhive, $database_bbhive, $query_last_id);
		$proximaOrdem 	= $row_last_id['bbh_cam_ind_ordem'] > 0 ? $row_last_id['bbh_cam_ind_ordem']+1 : 1;
	//--
	
		$proximaOrdem 	= $proximaOrdem;
		$nome_campo 	= "bbh_ind_cam_";
		$titulo			= ($_POST['bbh_cam_ind_titulo']);

		$query_campo_existente = sprintf("SELECT * FROM bbh_campo_indicio WHERE bbh_cam_ind_curinga = %s AND bbh_cam_ind_titulo = %s AND bbh_cam_ind_codigo = %s", GetSQLValueString($bbhive, ($_POST['bbh_cam_ind_curinga']), "text"), GetSQLValueString($bbhive, $titulo, "text"),GetSQLValueString($bbhive, $_POST['bbh_ind_codigo'], "int"));
	...
	$insertSQL = sprintf("INSERT INTO bbh_campo_indicio (bbh_cam_ind_nome, bbh_cam_ind_titulo, bbh_cam_ind_tipo, bbh_cam_ind_curinga, bbh_cam_ind_descricao, bbh_cam_ind_tamanho, bbh_cam_ind_default, bbh_cam_ind_ordem) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)",
  ...
  $ultimoId= mysqli_insert_id($bbhive);
	  ...
	  $nome_campo 	= "bbh_ind_cam_" . $ultimoId;
	  //--
		 $updateSQL = "UPDATE bbh_campo_indicio SET bbh_cam_ind_nome='$nome_campo' WHERE bbh_cam_ind_codigo=$ultimoId";
 	...
 	if($_POST['cadastrado'] == 1){	  //Se a tabela já tiver sido criada, ela deve dar um ALTER TABLE ADD	
		...
		$alterTable = "ALTER TABLE bbh_indicio ADD " . descobreValorDinamico($nome_campo, $_POST['bbh_cam_ind_tipo'],$tamanho,0,$valor_padrao);
        list($res, $rows, $totalRows) = executeQuery($bbhive, $database_bbhive, $alterTable);
	}
  }
  
	 /*echo "<var style='display:none'>showHome('includes/home.php','conteudoGeral', 'perfil/index.php?perfil=1&amp;menuEsquerda=1|departamentos/campos_tabela/index.php','menuEsquerda|colCentro')</var>";*/
...
//--
	$query_proximoId = "SELECT (COALESCE(bbh_cam_ind_codigo,0)+1) as id FROM bbh_campo_indicio";
...
?>
<var style="display:none">txtSimples('tagPerfil', 'Cadastro campos')</var>
<table width="98%" border="0" align="center" cellpadding="0" cellspacing="0" class="verdana_10">
  <tr>
    <td height="26" background="/e-solution/servicos/bbhive/images/barra_horizontal.jpg" class="verdana_11_bold"><img src="/e-solution/servicos/bbhive/images/detalhamento.gif" width="16" height="16" align="absmiddle" /> Gerenciamento campos - <?php echo $_SESSION['adm_componentesNome']; ?></td>
  </tr>
</table>
...
<table width="98%" border="0" align="center" cellpadding="2" cellspacing="0" class="borderAlljanela" style="margin-top:20px;">
  <tr class="legandaLabel11">
    <td height="26" colspan="2" background="/e-solution/servicos/bbhive/images/barra_horizontal2.jpg" class="verdana_11_bold">&nbsp;Novo campo de detalhamento - Itens</td>
  </tr>
  <tr class="legandaLabel11">
    <td width="175" height="25" align="right" class="verdana_11_bold">Titulo :&nbsp;</td>
    <td align="left"><input name="bbh_cam_ind_titulo" type="text" class="back_Campos" id="bbh_cam_ind_titulo" size="60"></td>
  </tr>
  <tr class="legandaLabel11">
    <td height="25" align="right" class="verdana_11_bold">Tipo :&nbsp;</td>
    <td align="left">
    <select name="bbh_cam_ind_tipo" id="bbh_cam_ind_tipo" class="back_Campos" onChange="showCamposCentral(this);">
  ...
  <tr style="display:none;background:#F7F7F7;background:#F7F7F7;" id="Tamanho">
    ...
    <td align="left" class="verdana_11"><label>
      <input type="text" name="bbh_cam_ind_tamanho" id="bbh_cam_ind_tamanho" class="back_Campos" >
    </label></td>
  </tr>
  ...
  <tr style="display:none;background:#F7F7F7;" id="VlPadrao">
    <td align="right" class="verdana_11_bold">Valor padr&atilde;o :&nbsp; </td>
    <td align="left" class="verdana_11"><label>
      <input name="bbh_cam_ind_default" type="text" class="back_Campos" id="bbh_cam_ind_default" size="30">
    </label></td>
  </tr>
  ...
  <tr style="display:none;background:#F7F7F7;" id="texto_longoDefault">
    ...
    <td align="left" class="verdana_11"><textarea name="bbh_cam_ind_defaultLongo" id="bbh_cam_ind_defaultLongo" cols="30" rows="3" class="back_input"></textarea></td>
  </tr>
  ...
  <tr class="legandaLabel11">
    ...
    <td align="left"><input type="text" name="bbh_cam_ind_curinga" id="bbh_cam_ind_curinga" class="back_Campos"></td>
  </tr>
  <tr class="legandaLabel11">
    <td height="25" align="right" class="verdana_11_bold">Observa&ccedil;&atilde;o :&nbsp;</td>
    <td align="left"><textarea name="bbh_cam_ind_descricao" id="bbh_cam_ind_descricao" cols="45" rows="5" class="back_Campos"></textarea></td>
  </tr>
  <tr class="legandaLabel11">
    ...
    <td align="left">
	  ...
	  <?php
if($tabelaCriada == 0){
	$alerta = "return validaForm('form1', 'bbh_cam_ind_titulo|Escolha o t&iacute;tulo do campo, bbh_cam_ind_tipo|Escolha o tipo do campo', document.getElementById('acaoForm').value)";
}else{
	$alerta = "if(confirm('O tipo do campo não poderá ser alterado posteriormente. Deseja continuar?')){return validaForm('form1', 'bbh_cam_ind_titulo|Escolha o t&iacute;tulo do campo, bbh_cam_ind_tipo|Escolha o tipo do campo', document.getElementById('acaoForm').value)}";
}
	  ?>
      ...
      <input name="bbh_ind_codigo" type="hidden" id="bbh_ind_codigo" value="<?php echo $proximoId; ?>"></td>
  </tr>
  <tr class="legandaLabel11">
    <td height="25" align="right" class="verdana_11_bold">&nbsp;</td>
    <td align="left">&nbsp;</td>
