...
if($tabelaCriada==1){
	...
	$bbh_pro_codigo 		= $_POST['bbh_pro_codigo'];
		...
		$query_campos_detalhamento = "SELECT * FROM bbh_campo_detalhamento_protocolo where bbh_cam_det_pro_disponivel ='1'and bbh_cam_det_pro_preencher_apos_receber='1' and bbh_cam_det_pro_fixo='0'";
		...
		//-----------------------------------------------------------------------
			...
			while($row_campos_detalhamento = mysqli_fetch_assoc($campos_detalhamento)){
				...
				//--
					...
					}else if($tipoDeCampo == "data")
					{
					...
					}else if($tipoDeCampo == "correio_eletronico" || $tipoDeCampo == "endereco_web" or $tipoDeCampo == "lista_opcoes" || $tipoDeCampo == "lista_dinamica" || $tipoDeCampo == "texto_longo" || $tipoDeCampo == "texto_simples"){
						...
						//--
						$valores .= " $nomeFisico = '" . mysqli_fetch_assoc($_POST[$nomeFisico]) . "'" . ",";
		...
		//--
		echo "<span style='margin-right:25px;'>Dados atualizados com sucesso!</span>";
}
?>