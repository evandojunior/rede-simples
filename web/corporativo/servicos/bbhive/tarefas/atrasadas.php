...
function enviaParaEstaPessoa($origem, $para, $assunto, $smtp, $usuario, $senha, $html){
...
//--Dados para envio do email-------------------------------------------------------------------------
		$origem 		= "bbhive@blackbee.com.br";
		$smtp			= "smtp.backsite.com.br";
		$usuario		= "robson";
		$senha			= "lia@190200";
		$subject 		= "Confirmação BBHIVE";
		$assunto  		= "Sistema de aviso automático de atividades atrasadas";
//-----------------------------------------------------------------------------------------------------
...
//---------------------------------------------------------------------------------------------------		 
	...
	while($row_sql = mysqli_fetch_assoc($sqlAtrasadas)){		
		...
		$emailDestino	= "julia@backsite.com.br";//$row_sql['bbh_usu_identificacao'];
		...
		if($proximoUsuario != $usuCod){
			...
			//--Envia mensagem
			enviaParaEstaPessoa($origem, $para, $assunto, $smtp, $usuario, $senha, $conteudoMsg);
			//--Zera variáveis
			$rodapeHTML 	= "";
			$cabecalhoHTML 	= "";
			$html			= "";
