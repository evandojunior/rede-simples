...
$ondeJar = $_SESSION['EndURL_BBPASS']."/servicos/bbpass/modulos_autenticacao/modulos/biometria/";
...
function verificaAcesso($database_bbpass, $bbpass, $chave, $email){
	$query_Login = "SELECT * FROM bbp_adm_lock_biometria WHERE bbp_adm_lock_bio_email='$email' AND bbp_adm_lock_bio_chave='$chave'";
 ...
 if(isset($_POST) && isset($_POST['bbp_adm_lock_bio_email'])){
 $idAplicacao= $_POST['idAplicacao'];
  $idLock	 = $_POST['idLock'];
  $chave     = $_POST['bbp_adm_lock_bio_chave'];
  $email	 = isset($_SESSION['MM_Email_Padrao'])?$_SESSION['MM_Email_Padrao']:$_POST['bbp_adm_lock_bio_email'];
		
	$result  = verificaAcesso($database_bbpass, $bbpass, $chave, $email);
	$liberado= 0;
...
<?php  
 if(@!array_key_exists($idLock,$_SESSION['modulosLiberados'])){ 
	...
	require_once("biometria/formulario.php");
	exit;
 } else { 
