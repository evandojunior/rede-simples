...
if(isset($_POST['atualizaLoginSenha'])){
	...
	//--
	$query_Login = "SELECT * FROM bbp_adm_lock_loginchave WHERE bbp_adm_lock_log_chave='".$_SESSION['MM_Email_Padrao']."' AND bbp_adm_lock_log_senha='$senhaAtual' AND bbp_adm_lock_log_ativo='1'";
    list($Login, $row_Login, $totalRows_Login) = executeQuery($bbpass, $database_bbpass, $query_Login);
	
		if($totalRows_Login > 0){
			//--UPDATE
		  	$updateSQL = "UPDATE bbp_adm_lock_loginchave SET bbp_adm_lock_log_senha='".$novaSenha."' Where bbp_adm_lock_log_chave='".$_SESSION['MM_Email_Padrao']."' AND bbp_adm_lock_log_codigo=".$row_Login['bbp_adm_lock_log_codigo'];
			...
			?>
				...
				<script type="text/javascript">
				  ...
				  location.href="login_chave.php?id=<?php echo $_POST['codigo']; ?>";
				</script>
			<?php
			exit;
		}
...
function verificaAcesso($database_bbpass, $bbpass, $Email, $Senha){
	$query_Login = "SELECT * FROM bbp_adm_lock_loginchave WHERE bbp_adm_lock_log_chave='$Email' AND bbp_adm_lock_log_senha='$Senha' AND bbp_adm_lock_log_ativo='1'";
    list($Login, $row_Login, $totalRows_Login) = executeQuery($bbpass, $database_bbpass, $query_Login);

		if($totalRows_Login>0){
			$_SESSION['MM_Email_Padrao'] = isset($_SESSION['MM_Email_Padrao'])?$_SESSION['MM_Email_Padrao']:$row_Login['bbp_adm_lock_log_chave'];
 ...
 $idAplicacao= $_POST['idAplicacao'];
  $idLock	 = $_POST['idLock'];
	$Email	 = isset($_SESSION['MM_Email_Padrao'])?$_SESSION['MM_Email_Padrao']:$_POST['bbp_adm_lock_log_chave'];
...
<?php  
 if(@!array_key_exists($idLock,$_SESSION['modulosLiberados'])){
	require_once("login_chave/formulario.php"); 
 } else { 
    //redireciona para página de sucesso
 	?>
    <script type="text/javascript" src="/servicos/bbpass/includes/javascript_backsite/ajax/projeto.js"></script>
    <form id="formAlteraLoginSenha" name="formAlteraLoginSenha" method="post" action="login_chave.php">
