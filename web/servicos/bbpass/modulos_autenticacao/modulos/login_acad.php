...
$caminhoLock = $rootPath."/Connections/cidadao.php";

require_once($caminhoLock);
require_once(str_replace("cidadao.php","bbpass.php",$caminhoLock));

// FUNÇÕES GLOBAIS
include($rootPath."/../database/config/globalFunctions.php");

function verificaAcesso($database_cidadao, $cidadao, $Email, $Senha){
	$query_Login = "SELECT cid_codigo, cid_nome, cid_sexo, cid_email FROM cidadao WHERE cid_email='$Email' AND cid_senha='$Senha'";
...
<?php  
 ...
 if(@!array_key_exists($idLock,$_SESSION['modulosLiberados'])){
	require_once("loginAcad/formulario.php"); 
 } else { 
