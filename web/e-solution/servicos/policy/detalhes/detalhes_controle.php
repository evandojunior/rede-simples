...
$CondicaoSQL = "1=1";
	if(isset($_GET['quemdet'])){
		$CondicaoSQL = " and a.pol_aud_usuario='".$_GET['quemdet']."'";
	} elseif(isset($_GET['relevanciadet'])){
		$CondicaoSQL = " and a.pol_aud_relevancia=".$_GET['relevanciadet'];
	} elseif(isset($_GET['dia'])){
		$CondicaoSQL = " and DATE_FORMAT(pol_aud_momento, '%d%m%Y')=".$_GET['dia'];
	} elseif(isset($_GET['ip'])){
		$CondicaoSQL = " and a.pol_aud_ip='".$_GET['ip']."'";
	} elseif(isset($_GET['acao'])){
		$CondicaoSQL = " and a.pol_aud_acao='".$_GET['acao']."'";
	} else { echo "Ocorreu um erro inesperado, entre em contato com o administrador do sistema."; }

	 $Sql = "
	 SELECT 
		DATE_FORMAT(a.pol_aud_momento,'%d/%m/%Y %H:%i:%s') AS momento, 
		a.pol_aud_usuario, 
		apl.pol_apl_nome,
		a.pol_aud_acao, 
		a.pol_aud_ip, 
		a.pol_aud_nivel, 
		a.pol_aud_relevancia, 
		a.pol_aud_codigo,
		SUM( apo.apo_tempo_execucao ) * 60 'tempo'
		
	
	FROM pol_auditoria a
	RIGHT JOIN pol_aplicacao apl		ON a.pol_apl_codigo = apl.pol_apl_codigo
	INNER JOIN pol_apontamento apo	ON a.pol_aud_acao = apo.pol_aud_acao
	
	WHERE 
		apl.pol_apl_codigo = $colname_codigo $CondicaoSQL 
		
	GROUP BY a.pol_aud_acao
	
	ORDER BY 
		a.pol_aud_momento DESC";
...
function verificaApontamento($database_policy, $policy, $idApl, $acao){
	$sqlApont = "select ap.apo_tempo_execucao from pol_apontamento as ap where ap.pol_apl_codigo = $idApl and ap.pol_aud_acao = '".mysqli_real_escape_string($acao)."'";
	mysqli_select_db($policy, $database_policy);
...
select a.pol_aud_usuario, a.pol_apl_codigo,
 sum(apo.apo_tempo_execucao)*60 as total
  from pol_auditoria as a 
   inner join pol_apontamento as apo on a.pol_aud_acao = apo.pol_aud_acao
    where 
	a.pol_apl_codigo = $colname_codigo $CondicaoSQL
    group by a.pol_aud_usuario, a.pol_apl_codigo
";
$exec_query = mysqli_query($policy, $sql);
...
<tr align="left" id="linha_<?php echo $row_detalhegeral['pol_aud_codigo']; ?>" class="verdana_11">
  ...
  <td style="border-bottom:1px dotted #003333;" height="24" align="center">
  <?php 
  // $t+= $j =verificaApontamento($database_policy, $policy, $_GET['pol_apl_codigo'], $a); 
  //echo $j;   
  $t += $row_detalhegeral['tempo'];
  echo substr( sec2hms($row_detalhegeral['tempo']), 0, -3);
  ?>
  &nbsp;
  <label style="font-size:9px;">min</label></td>
    ...
    <?php } ?>
  <?php $a++;} while ($row_detalhegeral = mysqli_fetch_assoc($detalhegeral));
  }
  ?>
  ...
  <tr>
    <td>&nbsp;</td>
    <td align="right"><?php if($totalRows_detalhegeral < 1){ echo "Nenhum resultado."; } ?>
    <strong>Total</strong></td>
    <td align="right"><strong><?php echo substr( sec2hms($t), 0, -3); ?>&nbsp;
