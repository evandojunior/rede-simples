...
$dadosGrafico = mysql_query($query_dadosGrafico, $policy) or die(mysql_error());
$row_dadosGrafico = mysql_fetch_assoc($dadosGrafico);
$totalRows_dadosGrafico = mysql_num_rows($dadosGrafico);
...
$dadosPolitica = mysql_query($query_dadosPolitica, $policy) or die(mysql_error());
$row_dadosPolitica = mysql_fetch_assoc($dadosPolitica);
$totalRows_dadosPolitica = mysql_num_rows($dadosPolitica);
...
//select dos dados
	...
	$Sql = "SELECT date_format(pol_aud_momento,'%d/%m/%Y') AS pol_aud_momento, pol_aud_usuario, pol_aud_acao, pol_aud_ip, pol_aud_nivel, pol_aud_relevancia, pol_aud_codigo, pol_apl_nome, count(pol_aud_momento) as Total FROM pol_auditoria RIGHT JOIN pol_aplicacao ON pol_auditoria.pol_apl_codigo = pol_aplicacao.pol_apl_codigo WHERE 1=1 $Condicoes GROUP BY  date_format(pol_aud_momento,'%d/%m/%Y') Order by pol_aud_momento ASC";
	...
	$total				= 0;
		...
		do{
			array_push($texto_linha,$row_detalhegeral['pol_aud_momento']);
			array_push($texto_coluna,$row_detalhegeral['pol_aud_momento']);
