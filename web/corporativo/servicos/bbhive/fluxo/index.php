...
<table width="98%" border="0" align="center" cellpadding="0" cellspacing="0" class="verdana_11 color">
  <tr>
    <td><?php require_once("cabecalhoModeloFluxo.php"); ?>
      <div style="align:center; float:right; "><strong><span id="statusFluxo"><?php echo $row_Fluxos['concluido']; ?>%</span></strong><?php
	    if(($row_Fluxos['bbh_usu_codigo']==$_SESSION['usuCod']) && ($row_Fluxos['bbh_flu_finalizado']=="1") && ($row_Fluxos['atividades']!=$row_Fluxos['finalizadas'])){
		...
		$acaoReabre	= "OpenAjaxPostCmd('".$acaoDestino."','menLoad','&1=1','&nbsp;Processando informações...','menLoad','2','2');";
			...
			echo '&nbsp;&nbsp;<a href="#@" onClick="'.$acaoReabre.'"><img src="/corporativo/servicos/bbhive/images/cadeado.gif" border="0" align="absmiddle" title="Reabrir '.$_SESSION['FluxoNome'].'" /></a>';
		}
	  ?></div>
    </td>
  </tr>
...
<?php 
if( isset( $_GET['paginaTarefas'] ) )
{
	$bbh_ati_codigo = $_GET['bbh_ati_codigo'];
	require_once("../tarefas/acao/menu_icones.php");
}
else
{
	require_once("menu_icones.php");
}
?>
<?php
//--ARQUIVOS DO PROTOCOLO
//Descobre qual protocolo é deste fluxo
$sqlFluProt = "select bbh_pro_codigo from bbh_protocolos as p where p.bbh_flu_codigo = ".$bbh_flu_codigo." limit 1";
...
<table width="98%" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    <td width="595" height="33" style="border-left:#D7D7D7 solid 1px; background-image:url(/corporativo/servicos/bbhive/images/backTopII.jpg); background-repeat:no-repeat">
    <div id="titulo" style="width:117px; height:25px; margin-top:5px; vertical-align:middle;font-weight:bold;" class="verdana_12 color">&nbsp;<strong><?php echo $_SESSION['componentesNome']; ?></strong></div>
...
<form name="updateFluxo" id="updateFluxo" style="margin-top:5px;width:98%; align:center;" onsubmit="return false;">
   ...
   <table width="98%" align:center border="0" cellspacing="0" cellpadding="0" class="verdana_12 color">
      <tr>
        ...
        <?php if($row_Fluxos['bbh_flu_finalizado']=='0'){ ?>
        	    <a href="#" onclick="if(retiraEspacos(document.formFluxo.bbh_flu_titulo.value)==''){ alert('O título não deve ficar vazio!'); } else { document.updateFluxo.bbh_flu_titulo.value = document.formFluxo.bbh_flu_titulo.value; document.updateFluxo.bbh_flu_observacao.value = document.formFluxo.bbh_flu_observacao.value; <?php echo $acao; ?> }">
            	<img src="/corporativo/servicos/bbhive/images/salvar.gif" width="16" height="16" border="0" align="absmiddle" /></a>

                <input name="bbh_flu_titulo" type="text" id="bbh_flu_titulo" value="" style="display: none">
                <input name="bbh_flu_observacao" type="text" id="bbh_flu_observacao" value="" style="display: none">
        <?php } else { ?>    
  ...
  <tr>
    <td width="98%" height="200" valign="top" style="border-left:#D7D7D7 solid 1px; border-right:#EBEFF0 solid 1px; border-bottom:#D7D7D7 solid 1px" id="corpoFluxo" >
    <div id="atividade" 	class="show"><span class="verdana_12" id="loadAtividade">&nbsp;Carregando dados...</span></div>
    <div id="fluxo" 		class="hide"><span class="verdana_12">&nbsp;Carregando dados...</span></div>
    <div id="mensagem" 		class="hide"><span class="verdana_12">&nbsp;Carregando dados...</span></div>
    <div id="detalhamento" 	class="hide"><span class="verdana_12">&nbsp;Carregando dados...</span></div>
    <div id="indicio" 		class="hide"><span class="verdana_12">&nbsp;Carregando dados...</span></div>
...
$carregaPagina	= "OpenAjaxPostCmd('".$homeDestino."','atividade','&1=1','&nbsp;Carregando dados...','loadAtividade','2','1');";
	...
	foreach($_GET as $indice=>$valor){
		if($indice == "exibeAtividade" || $indice == "amp;exibeAtividade"){
			$carregaPagina = "escondeBloco('corpoFluxo','atividade', 'titulo', '&nbsp;Atividades', '/corporativo/servicos/bbhive/fluxo/atividades.php?bbh_flu_codigo=".$bbh_flu_codigo."')";
			break;
		}
		if($indice == "exibeMensagem" || $indice == "amp;exibeMensagem"){
			$carregaPagina = "escondeBloco('corpoFluxo','mensagem', 'titulo', '&nbsp;Mensagens', '/corporativo/servicos/bbhive/fluxo/mensagens.php?bbh_flu_codigo=".$bbh_flu_codigo."&caixaEntrada=true&bbh_flu_codigo=".$bbh_flu_codigo."')";
			break;
		}
		if($indice == "exibeIndicios" || $indice == "amp;exibeIndicios"){
			$carregaPagina = "escondeBloco('corpoFluxo','indicio', 'titulo', '&nbsp;".$_SESSION['componentesNome']."', '/corporativo/servicos/bbhive/fluxo/indicios/regra.php?bbh_flu_codigo=".$bbh_flu_codigo."')";
			break;
		}
	}
?>
