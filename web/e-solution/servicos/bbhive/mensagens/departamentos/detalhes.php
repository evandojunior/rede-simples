...
$query_mensagens = "SELECT bbh_departamento.bbh_dep_nome, bbh_men_assunto, bbh_men_mensagem, bbh_men_data_recebida, bbh_men_codigo FROM bbh_mensagens INNER JOIN bbh_usuario ON bbh_usuario.bbh_usu_codigo = bbh_mensagens.bbh_usu_codigo_remet INNER JOIN bbh_departamento ON bbh_departamento.bbh_dep_codigo = bbh_usuario.bbh_dep_codigo WHERE bbh_departamento.bbh_dep_codigo = $codDpt ORDER BY date_format(bbh_mensagens.bbh_men_data_recebida,'%Y') DESC,date_format(bbh_mensagens.bbh_men_data_recebida,'%m') DESC ,date_format(bbh_mensagens.bbh_men_data_recebida,'%d') DESC, date_format(bbh_mensagens.bbh_men_data_recebida, '%H') DESC, date_format(bbh_mensagens.bbh_men_data_recebida, '%i') DESC, date_format(bbh_mensagens.bbh_men_data_recebida, '%s') DESC";
...
<table width="98%" border="0" align="center" cellpadding="0" cellspacing="0" class="verdana_10">
  <tr>
    ...
    <td width="21%" align="right" background="/e-solution/servicos/bbhive/images/barra_horizontal.jpg"><a href="#@" onClick="LoadSimultaneo('perfil/index.php?perfil=2|mensagens/departamentos/index.php','menuEsquerda|conteudoGeral');"><span class="verdana_11_bold color">.:&nbsp;Voltar&nbsp;:.</span></a>&nbsp;</td>
  </tr>
  <tr>
      ...
      <tr>
        <td colspan="2"><table width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr style="font-weight:bold; background-image:url(/e-solution/servicos/bbhive/images/barra_horizontal2.jpg)">
            <td height="27" colspan="4">&nbsp;<a href="#@" onClick="LoadSimultaneo('perfil/index.php?perfil=2|mensagens/usuarios/index.php','menuEsquerda|conteudoGeral');"><img src="/e-solution/servicos/bbhive/images/ger-departamento-16px.gif" alt="" width="12" height="12" border="0" align="absmiddle" /></a> Mensagens referente ao Departamento: <span class="color"> <?php echo $row_mensagens['bbh_dep_nome']; ?></span></td>
          </tr>
    <?php if ($totalRows_mensagens > 0) { // Show if recordset not empty ?>
		...
		list($remet, $row_remet, $totalRows_remet) = executeQuery($bbhive, $database_bbhive, $query_remet);
		  ...
		  ?>
          <tr onClick="LoadSimultaneo('perfil/index.php?perfil=2|/mensagens/mensagem.php?bbh_men_codigo=<?php echo $codMens; ?>&back=<?php echo $voltarURL; ?>&cdg=<?php echo "bbh_dep_codigo=".$codDpt; ?>','menuEsquerda|conteudoGeral');" id="u<?php echo $codMens; ?>" onMouseOver="ativaCor('u<?php echo $codMens; ?>');" onMouseOut="desativaCor('u<?php echo $codMens; ?>');">
