...
$acao = mysql_query($query_limit_acao, $policy) or die(mysql_error());
$row_acao = mysql_fetch_assoc($acao);

if (isset($_GET['totalRows_acao'])) {
  $totalRows_acao = $_GET['totalRows_acao'];
} else {
  $all_acao = mysql_query($query_acao);
  $totalRows_acao = mysql_num_rows($all_acao);
}
...
function verificaApontamento($database_policy, $policy, $idApl, $acao){
	$sqlApont = "select ap.apo_codigo from pol_apontamento as ap where ap.pol_apl_codigo = $idApl and ap.pol_aud_acao = '".mysql_real_escape_string($acao)."'";
	mysql_select_db($database_policy, $policy);
	$Apont 			 = mysql_query($sqlApont, $policy) or die(mysql_error());
	return mysql_num_rows($Apont);
}
?>
...
<?php 
			...
			<td align="center" style="border-bottom:dotted 1px #333333;"><?php if($row_acao['acessos']==0){echo "---";}else{echo $row_acao['acessos']; }?></td>
              ...
              <?php } while ($row_acao = mysql_fetch_assoc($acao)); ?>
