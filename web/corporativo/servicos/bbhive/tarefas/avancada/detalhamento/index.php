...
<table width="98%" border="0" align="center" cellpadding="0" cellspacing="0" class="verdana_11">
   <?php while($row_Campos = mysqli_fetch_assoc($Campos)){ 
   	  ...
   	  ?>   
      <tr>
        <td width="4%" height="23" align="center" bgcolor="#EFEFE7" class="verdana_11"><label>
          <input type="checkbox" name="chk_<?php echo $CodigoCampo; ?>" id="chk_<?php echo $CodigoCampo; ?>" onClick="javascript: if(this.checked==true){ computaSelecao(1);} else {computaSelecao(-1);}" />
        </label></td>
        <td width="32%" bgcolor="#FFFFFF" class="verdana_11">&nbsp;<?php echo $tituloCampo; ?><input name="nm_campo_<?php echo $CodigoCampo; ?>" id="nm_campo_<?php echo $CodigoCampo; ?>" type="hidden" value="<?php echo $nomeCampo; ?>">
...
<?php if($totalRows_Campos==0){?>
<var style="display:none">document.getElementById('temExportacao').style.display='none';</var>
<?php } ?>