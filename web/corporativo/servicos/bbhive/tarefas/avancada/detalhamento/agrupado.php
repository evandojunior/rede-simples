...
if($tabelaCriada == 1){
	$query_ConsultaTabela = "select $bbh_cam_det_flu_nome, length($bbh_cam_det_flu_nome) from bbh_modelo_fluxo_".$bbh_mod_flu_codigo."_detalhado inner join bbh_fluxo on bbh_modelo_fluxo_".$bbh_mod_flu_codigo."_detalhado.bbh_flu_codigo = bbh_fluxo.bbh_flu_codigo where bbh_flu_finalizado<>'1' AND length($bbh_cam_det_flu_nome) > 0 AND $bbh_cam_det_flu_nome is not null GROUP by $bbh_cam_det_flu_nome order by 1 ASC";
