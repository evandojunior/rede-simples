...
//include($_SESSION['caminhoFisico']."/e-solution/servicos/bbhive/includes/autentica.php");
	...
	$homeDestinoII	= '/e-solution/servicos/bbhive/departamentos/campos_tabela/excluir.php';
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
  ...
  $deleteSQL = sprintf("DELETE FROM bbh_campo_indicio WHERE bbh_cam_ind_codigo=%s",
                       GetSQLValueString($bbhive, $_POST['bbh_cam_ind_codigo'], "int"));
    list($deleteSQL, $fetch, $totalRows) = executeQuery($bbhive, $database_bbhive, $deleteSQL, $initResult = false);
...
if (isset($_GET['bbh_cam_ind_codigo'])) {
  $colname_campos_detalhamento = $_GET['bbh_cam_ind_codigo'];
}
$query_campos_detalhamento ="SELECT date_format(bbh_cam_ind_default,'%d/%m/%Y') as data,bbh_campo_indicio.* FROM bbh_campo_indicio WHERE bbh_cam_ind_codigo = $colname_campos_detalhamento";
...
<table width="98%" border="0" align="center" cellpadding="2" cellspacing="0" class="borderAlljanela">
  ...
  <tr class="legandaLabel11">
    ...
    <td align="left"><input name="bbh_cam_ind_titulo" type="text" class="back_Campos" id="bbh_cam_ind_titulo" value="<?php echo $row_campos_detalhamento['bbh_cam_ind_titulo']; ?>" size="60" disabled></td>
  </tr>
  <tr class="legandaLabel11">
    <td height="25" align="right" class="verdana_11_bold">Tipo :&nbsp;</td>
    <td align="left">
    <select name="bbh_cam_ind_tipo" id="bbh_cam_ind_tipo" class="back_Campos" disabled>
  ...
  <tr style="display:none;background:#F7F7F7;background:#F7F7F7;" id="Tamanho">
    ...
    <td align="left" class="verdana_11"><label>
      <input name="bbh_cam_ind_tamanho" type="text" class="back_Campos" id="bbh_cam_ind_tamanho" value="<?php echo $row_campos_detalhamento['bbh_cam_ind_tamanho']; ?>" disabled />
    </label></td>
  </tr>
    ...
    <?php
			if($is_float == true)
			{
				$valor_default = Real($row_campos_detalhamento['bbh_cam_ind_default']); ?>
				      <input name="bbh_cam_ind_default" type="text" class="back_Campos" id="bbh_cam_ind_default" size="30" value="<?php echo $valor_default; ?>" onKeyPress="return(MascaraMoeda(this,'.',',',event))" disabled>

 		 <?php }else if($is_number == true){ 
         	$valor_default = $row_campos_detalhamento['bbh_cam_ind_default']; ?>
          <input name="bbh_cam_ind_default" type="text" class="back_Campos" id="bbh_cam_ind_default" size="30" value="<?php echo $valor_default; ?>" onkeyup="SomenteNumerico(this);" disabled/>

			<?php }else{
				$valor_default = $row_campos_detalhamento['bbh_cam_ind_default']; ?>
                      <input name="bbh_cam_ind_default" type="text" class="back_Campos" id="bbh_cam_ind_default" size="30" value="<?php echo $valor_default; ?>" disabled/>
  ...
  <tr style="display:<?php echo $longo; ?>;background:#F7F7F7;" id="texto_longoDefault">
    ...
    <td align="left" class="verdana_11"><textarea name="bbh_cam_ind_defaultLongo" id="bbh_cam_ind_defaultLongo" cols="30" rows="3" class="back_Campos" disabled><?php echo $row_campos_detalhamento['bbh_cam_ind_default']; ?></textarea></td>
  </tr>
  ...
  <tr class="legandaLabel11">
    ...
    <td align="left"><input type="text" name="bbh_cam_ind_curinga" id="bbh_cam_ind_curinga" class="back_Campos" value="<?php echo $row_campos_detalhamento['bbh_cam_ind_curinga']; ?>" disabled></td>
  </tr>
  <tr class="legandaLabel11">
    <td height="25" align="right" class="verdana_11_bold">Observa&ccedil;&atilde;o :&nbsp;</td>
    <td align="left"><textarea name="bbh_cam_ind_descricao" id="bbh_cam_ind_descricao" cols="45" rows="5" class="back_Campos" disabled><?php echo $row_campos_detalhamento['bbh_cam_ind_descricao']; ?></textarea></td>
  </tr>
  <tr class="legandaLabel11">
    ...
    <td align="left">
      ...
      <input name="cadastra" type="button" class="back_input" id="cadastra" value="Excluir campo" style="cursor:pointer; background-image:url(/e-solution/servicos/bbhive/images/search.gif); color:#FFFFFF" onClick="return validaForm('form1', 'bbh_cam_ind_titulo|Escolha o t&iacute;tulo do campo, bbh_cam_ind_tipo|Escolha o tipo do campo', document.getElementById('acaoForm').value)"/>
