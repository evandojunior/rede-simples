...
// PARA O NOVO SEPARADOR PADRÃO
	...
	mysql_select_db($database_ged, $ged);
	$query_rsCampos = "SELECT * FROM ged_campo WHERE cam_tipo = 6";
...
?>
<form name="form1" id="form1" action="altera_separador.php" method="post">
<?php
do{
	$novaString = str_replace(',','<%SEPARA_VALOR%>',$row_rsCampos['cam_vl_default']);
	echo $row_rsCampos['cam_vl_default'];
	?>
	<br/>
	<input name="<?=$row_rsCampos['cam_nome']?>" id="<?=$row_rsCampos['cam_nome']?>" value="<?=$novaString?>" size="200" />
<?php
